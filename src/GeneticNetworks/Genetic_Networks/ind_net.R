...
ind.net <- function(d,n='number of observations',PLOT=FALSE,alpha=0.05,partial=TRUE,crit=0.3,fix.na=FALSE,fix.inf=FALSE){
  ...
  diag(R) <- 1
                                        #calculate tau
  tau <- -n*log((1-R^2))
  if (fix.na == TRUE){tau[is.na(tau)] <- 0}else{}
  ...
  if (PLOT==FALSE){
    ...
    gplot(R.ind,displaylabels=FALSE)
    detach(package:sna)
    return(R.ind)
  }
}
